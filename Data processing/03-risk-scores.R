...
f_risk_fair_estimation <- function(X_train,
  ...
  for (m in names(method_fairness_list)) {
    ...
    r.fair <- fairml::fgrrm(y_train, 
                            ...
                            definition = m, 
                            family = "binomial", 
                            lambda = 0 #comma removed might be an error
    )
...
# ---------------------------------------------------------------------------- #
# ROC / AUC evaluation — logistic risk score (risk_score_log)
# ---------------------------------------------------------------------------- #

cat("\n=== ROC / AUC: risk_score_log vs.", outcome, "===\n")

roc_log <- roc(
  response  = as.numeric(as.character(db_risk[[outcome]])),
  predictor = as.numeric(db_risk$risk_score_log)
)

auc_log <- auc(roc_log)
cat("AUC-ROC (Logistic Ridge, risk_score_log):", round(auc_log, 4), "\n")

opt <- coords(roc_log, "best", best.method = "youden",
              ret = c("threshold", "sensitivity", "specificity"))

cat("Optimal Threshold (Youden's J):  ", round(opt[["threshold"]], 4), "\n")
cat("Sensitivity at Optimal Threshold:", round(opt[["sensitivity"]], 4), "\n")
cat("Specificity at Optimal Threshold:", round(opt[["specificity"]], 4), "\n")

roc_plot_path <- file.path(wd_path, "roc_logistic_ridge.png")
png(filename = roc_plot_path)
plot(roc_log, main = "ROC Curve for Logistic Ridge Regression (risk_score_log)",
     col = "blue", lwd = 2)
points(opt[["specificity"]], opt[["sensitivity"]], col = "green", pch = 16)
text(opt[["specificity"]], opt[["sensitivity"]],
     labels = paste("Threshold:", round(opt[["threshold"]], 4)),
     pos = 4, col = "green")
dev.off()
cat("Saved ROC plot to:", roc_plot_path, "\n")
